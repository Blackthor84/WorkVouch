        ...
        <div className="flex flex-1 items-center justify-between gap-4 max-w-7xl mx-auto min-w-0">
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
            <div className="hidden md:flex items-center gap-2">
              ...
              {!isLoading && isAuthenticated && (
                <>
                  <Link href="/dashboard" className="text-white border border-white/80 px-4 py-2 rounded hover:bg-white/20 transition whitespace-nowrap">Dashboard</Link>
                  <Link href="/my-jobs" className="text-white border border-white/80 px-4 py-2 rounded hover:bg-white/20 transition whitespace-nowrap">Jobs</Link>
                  <Link href="/profile" className="text-white border border-white/80 px-4 py-2 rounded hover:bg-white/20 transition whitespace-nowrap">Profile</Link>
                  <button
                    type="button"
                    onClick={handleLogout}
                    className="text-white border border-white/80 px-4 py-2 rounded hover:bg-white/20 transition font-semibold whitespace-nowrap"
                  >
                    Logout
                  </button>
                </>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-14 left-0 right-0 bg-blue-700 border-t border-blue-600 space-y-2 py-4 px-4 z-50">
          ...
          <div className="pt-2 space-y-2">
            ...
            {!isLoading && isAuthenticated && (
              <>
                <Link href="/dashboard" className="block py-2 hover:text-gray-300 transition">Dashboard</Link>
                <Link href="/my-jobs" className="block py-2 hover:text-gray-300 transition">Jobs</Link>
                <Link href="/profile" className="block py-2 hover:text-gray-300 transition">Profile</Link>
                <button
                  type="button"
                  onClick={() => { handleLogout(); setMobileMenuOpen(false); }}
                  className="block w-full text-white border border-white/80 px-4 py-2 rounded hover:bg-white/20 transition font-semibold text-center"
                >
                  Logout
                </button>
              </>
