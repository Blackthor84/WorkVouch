# WorkVouch Landing Page Copy

**Complete Landing Page Content**

## Hero Section

### Headline
**"Your Work. Verified by the People Who Worked With You."**

### Subheading
**"WorkVouch helps you prove your experience using real coworker references — fast, simple, and trusted. Perfect for law enforcement, security, hospitality, and retail professionals."**

### Primary CTA
**"Get Started Free"** (Button: Primary blue, large, prominent)

### Secondary CTA
**"See How It Works"** (Link: Text link, smaller)

### Visual
- Hero image: Diverse professionals in work settings
- Trust Score visualization (animated)
- Mobile-responsive layout

---

## Feature Blocks

### Feature 1: Peer-Verified References
**Headline**: "Verified by Real Coworkers"
**Description**: "No fake references. Every job on your profile is verified by people who actually worked with you. Build trust with employers through authentic peer verification."
**Icon**: Checkmark/Verification badge

---

### Feature 2: Trust Score System
**Headline**: "Your Trust Score Speaks for Itself"
**Description**: "A 0-100 score that shows how verified and trustworthy your career profile is. Built from verified jobs, peer references, and reference quality. Employers trust it."
**Icon**: Score gauge/Chart

---

### Feature 3: Employer Instant Checks
**Headline**: "Employers Verify in Seconds"
**Description**: "Employers can instantly verify your work history, see your peer references, and check your Trust Score. No more waiting weeks for background checks."
**Icon**: Clock/Speed

---

### Feature 4: Secure Storage
**Headline**: "Your Data, Secured"
**Description**: "Your career profile is stored securely with enterprise-grade encryption. You control who sees what. Your privacy is our priority."
**Icon**: Lock/Security

---

### Feature 5: Cross-Industry Support
**Headline**: "Built for Your Industry"
**Description**: "Tailored for law enforcement, security, hospitality, and retail. Industry-specific fields, certifications, and verification processes designed for your field."
**Icon**: Industry/Building

---

### Feature 6: Mobile-Ready
**Headline**: "Access Anywhere, Anytime"
**Description**: "Manage your profile, request references, and check your Trust Score from your phone. Full mobile app coming soon."
**Icon**: Mobile/Phone

---

## Industry-Specific Callouts

### Law Enforcement
**Headline**: "Built for Law Enforcement Professionals"
**Description**: "Verify your service history with peer-verified references from fellow officers. Showcase your certifications, departments, and experience in a trusted, verified format."
**CTA**: "Join Law Enforcement Professionals"

---

### Security
**Headline**: "Trusted by Security Professionals"
**Description**: "Security companies trust WorkVouch to verify candidate experience. Build your verified security career profile with licenses, clearances, and peer references."
**CTA**: "Start Your Security Profile"

---

### Hospitality
**Headline**: "Perfect for Hospitality Workers"
**Description**: "Hotels and restaurants use WorkVouch to verify experience. Showcase your front desk, housekeeping, and guest service skills with peer-verified references."
**CTA**: "Build Your Hospitality Profile"

---

### Retail
**Headline**: "Retail Professionals Trust WorkVouch"
**Description**: "Retailers verify candidate experience instantly. Show your POS experience, cash handling, and customer service skills with verified references from coworkers."
**CTA**: "Create Your Retail Profile"

---

## Benefits Section

### Benefit 1: Stand Out to Employers
**"Employers trust verified profiles. Your WorkVouch profile shows you're serious about your career and have the references to back it up."**

### Benefit 2: Build Your Reputation
**"Your Trust Score grows as you add verified jobs and receive references. Build a reputation that follows you throughout your career."**

### Benefit 3: Save Time
**"No more waiting weeks for background checks. Employers can verify your experience in seconds through your WorkVouch profile."**

### Benefit 4: Control Your Narrative
**"You control your career profile. Add jobs, request references, and build your Trust Score on your terms."**

---

## Social Proof Section

### Testimonial 1
**Quote**: "WorkVouch helped me land my dream job. The employer could verify my experience instantly, and my Trust Score showed I was trustworthy."
**Author**: "Sarah M., Law Enforcement Professional"
**Location**: "Los Angeles, CA"

### Testimonial 2
**Quote**: "As an employer, WorkVouch saves us weeks of verification time. We can see verified work history and peer references in seconds."
**Author**: "John D., Hiring Manager"
**Company**: "Security Solutions Inc."

### Testimonial 3
**Quote**: "I love how easy it is to request references from coworkers. My Trust Score keeps growing, and employers notice."
**Author**: "Maria R., Hospitality Professional"
**Location**: "Miami, FL"

---

## Trust Indicators

### Stats
- **"10,000+ Verified Profiles"**
- **"50,000+ Peer References"**
- **"500+ Employers Using WorkVouch"**
- **"4.8/5 Average Trust Score"**

### Security Badges
- "Enterprise-Grade Security"
- "GDPR Compliant"
- "CCPA Compliant"
- "SOC 2 Certified" (if applicable)

---

## Pricing Preview

### Quick Overview
**"Start Free. Upgrade When You're Ready."**

**Starter (Free)**
- Unlimited jobs
- 3 references per job
- Basic Trust Score

**Pro ($8.99/month)**
- 10 references per job
- AI résumé tools
- ATS optimization

**Elite ($19.99/month)**
- Unlimited references
- Verified badge
- AI Career Coach

**CTA**: "View Full Pricing" (Link to pricing page)

---

## Final CTA Section

### Headline
**"Ready to Build Your Verified Career Profile?"**

### Subheading
**"Join thousands of professionals who trust WorkVouch to verify their work history."**

### Primary CTA
**"Get Started Free"** (Button: Large, prominent)

### Secondary Text
**"No credit card required. Set up your profile in minutes."**

---

## SEO Keywords

### Primary Keywords
- career verification
- peer references
- trust score app
- resume validation
- coworker validation
- job history verification
- verified work history
- peer-verified career
- employment verification
- background check alternative

### Long-Tail Keywords
- how to verify work history with coworkers
- peer-verified career platform
- trusted career profile builder
- verified employment references
- coworker verification system
- trust score for job seekers
- verified resume platform
- peer reference verification

### Industry-Specific Keywords
- law enforcement career verification
- security industry references
- hospitality work verification
- retail employment verification
- verified security career
- peer-verified law enforcement profile

---

## Meta Tags

### Title Tag
**"WorkVouch - Your Work. Verified by Coworkers. Trust Score Career Platform."**

### Meta Description
**"Build your verified career profile with peer references. Get a Trust Score that employers trust. Perfect for law enforcement, security, hospitality, and retail professionals. Start free."**

### Open Graph Tags
- **og:title**: "WorkVouch - Verified Career Profiles with Trust Scores"
- **og:description**: "Build your verified career profile with peer references. Get a Trust Score that employers trust."
- **og:image**: WorkVouch logo or hero image
- **og:url**: https://workvouch.com

---

## Call-to-Action Variations

### Primary CTAs
- "Get Started Free"
- "Create Your Profile"
- "Build Your Trust Score"
- "Join WorkVouch"

### Secondary CTAs
- "See How It Works"
- "View Pricing"
- "Learn More"
- "See Examples"

### Industry-Specific CTAs
- "Join Law Enforcement Professionals"
- "Start Your Security Profile"
- "Build Your Hospitality Profile"
- "Create Your Retail Profile"

---

**All copy is optimized for conversion, mobile-responsive, and aligned with WorkVouch's brand voice: professional, trustworthy, and helpful.**
