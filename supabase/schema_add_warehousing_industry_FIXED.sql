-- ============================================================================
-- ADD WAREHOUSING & LOGISTICS INDUSTRY (FIXED VERSION)
-- ============================================================================
-- This version creates the enum if it doesn't exist, then adds 'warehousing'
-- ============================================================================

-- Step 1: Create industry_type enum if it doesn't exist, or add 'warehousing' if it does
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Create the enum with all values including warehousing
  ...
  ELSE
    -- Enum exists, check if 'warehousing' value already exists
    IF NOT EXISTS (
      SELECT 1 FROM pg_enum 
      WHERE enumlabel = 'warehousing' 
      AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'industry_type')
    ) THEN
      -- Add 'warehousing' to existing enum
      ALTER TYPE industry_type ADD VALUE 'warehousing';
    END IF;
  END IF;
END $$;

-- Step 2: Add industry column to profiles table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Step 3: Add warehouse-specific fields to profiles table
DO $$ 
BEGIN
...
-- Step 4: Create indexes for warehouse fields
...
-- Step 5: Create or replace handle_new_user function to support warehousing industry
...
BEGIN
  ...
  RAISE NOTICE 'Warehousing industry has been added to industry_type enum.';
END $$;
