...
export interface WorkHistoryEntryWithDates {
  company: string;
  start_date: string;
...
/**
 * For one employee's parsed resume work_history, find other employees in the same org
 * whose work history overlaps (same company_normalized, overlapping dates).
...
export function computePeerSuggestions(
  ...
  for (const other of otherEmployees) {
    ...
    for (const src of sourceCompanies) {
      for (const their of other.parsedWorkHistory) {
