...
import { getRehireData, getTrustScoresForEmployer } from '@/lib/actions/employer/analytics'
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (employerAccountTyped.plan_tier !== 'pro' && employerAccountTyped.plan_tier !== 'enterprise') {
      return NextResponse.json(
    ...
    if (exportType === 'rehire') {
      ...
      csvData = convertToCSV(rehireData, ['workerEmail', 'rehireCount', 'lastHired'])
      filename = `rehire-data-${new Date().toISOString().split('T')[0]}.csv`
    } else if (exportType === 'trust-scores') {
      const trustScores = await getTrustScoresForEmployer(employerAccountTyped.id)
