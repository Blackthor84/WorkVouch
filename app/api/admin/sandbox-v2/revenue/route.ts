...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: rows } = await sb().from("sandbox_revenue").select("mrr, churn_rate").eq("sandbox_id", sandboxId).order("created_at", { ascending: false }).limit(1);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { count } = await sb().from("sandbox_employers").select("id", { count: "exact", head: true }).eq("sandbox_id", sandbox_id);
    const employer_count = count ?? 0;
    const mrr = employer_count * PLAN_VALUE;
    const { error } = await sb().from("sandbox_revenue").insert({ sandbox_id, mrr, churn_rate });
