...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: session } = await admin.from("simulation_sessions")
    ...
    const { data: profileIds, error: idError } = await admin.rpc("get_simulation_profile_ids_by_session", {
    ...
    const { data: purgeResult, error: purgeError } = await admin.rpc("purge_simulation_session", {
      p_session_id: sessionId,
    });
