  ...
  try {
    ...
    if (scope === "global") {
      const [p, e, r, i] = await Promise.all([
        admin.from("profiles").select("id", { count: "exact", head: true }).eq("is_simulation", false),
        admin.from("employment_records").select("id", { count: "exact", head: true }).eq("is_simulation", false),
        admin.from("employment_references").select("id", { count: "exact", head: true }).eq("is_simulation", false),
        admin.from("intelligence_snapshots").select("id", { count: "exact", head: true }).eq("is_simulation", false),
      ]);
      ...
      const { data: inserted, error: insErr } = await admin.from("data_density_snapshots").insert(insertRow).select("id, snapshot_at, profiles_count, employment_records_count, references_count, intelligence_rows_count").single();
    ...
    if (scope === "session" && scopeId) {
      const [p, e, r, i] = await Promise.all([
        admin.from("profiles").select("id", { count: "exact", head: true }).eq("simulation_session_id", scopeId),
        admin.from("employment_records").select("id", { count: "exact", head: true }).eq("simulation_session_id", scopeId),
        admin.from("employment_references").select("id", { count: "exact", head: true }).eq("simulation_session_id", scopeId),
        admin.from("intelligence_snapshots").select("id", { count: "exact", head: true }).eq("simulation_session_id", scopeId),
      ]);
      ...
      const { data: insertedSession, error: insErrSession } = await admin.from("data_density_snapshots").insert(insertRowSession).select("id, snapshot_at, profiles_count, employment_records_count, references_count, intelligence_rows_count").single();
