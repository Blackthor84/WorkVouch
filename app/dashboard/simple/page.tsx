      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 bg-background dark:bg-[#0D1117] min-h-screen">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {menuItems.map((item) => {
            ...
            return (
              <Card key={item.title} className="p-6 hover:shadow-lg transition-shadow cursor-pointer group">
                <Button
                  href={item.href}
                  variant="ghost"
                  className="w-full h-full p-0 flex flex-col items-start"
                >
                  <div className={`inline-flex h-12 w-12 items-center justify-center rounded-xl mb-4 ${item.color}`}>
