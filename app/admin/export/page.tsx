...
export default async function AdminExportPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8 bg-background dark:bg-[#0D1117] min-h-screen">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-grey-dark dark:text-gray-200 mb-2">Data Export & Compliance</h1>
        <p className="text-grey-medium dark:text-gray-400">Generate CSV exports. Admin only.</p>
      </div>
      <Card className="p-6">
        <ul className="space-y-3">
          {EXPORT_TYPES.map(({ type, label, filename }) => (
            <li key={type} className="flex items-center justify-between gap-4 border-b border-grey-background dark:border-[#374151] pb-3 last:border-0">
              <span className="text-grey-dark dark:text-gray-200">{label}</span>
