      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 bg-background dark:bg-[#0D1117] min-h-screen">
        ...
        <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          <p className="text-sm text-blue-900 dark:text-blue-200">
            ...
            <strong> Email Confirmed:</strong> {signups.filter(s => s.emailConfirmed).length} | 
            <strong> Profiles Created:</strong> {signups.filter(s => s.profileCreated).length}
          </p>
        </div>
            ...
            <Card key={signup.id} className="p-6">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {signup.industry && (
                    <p className="text-sm text-grey-medium dark:text-gray-400 mb-2">
                      Industry: <span className="font-medium">{signup.industry}</span>
                    </p>
                  )}

                  <div className="flex gap-4 text-xs text-grey-medium dark:text-gray-400 mt-3">
                    <span>
                      <strong>Signed Up:</strong> {signup.createdAt ? formatDateLong(signup.createdAt) : 'N/A'}
                    </span>
                    {signup.lastSignIn && (
                      <span>
                        <strong>Last Sign In:</strong> {formatDateLong(signup.lastSignIn)}
                      </span>
                    )}
                    {!signup.profileCreated && (
                      <Badge variant="warning" className="text-xs">No Profile</Badge>
                    )}
                  </div>
                </div>
              </div>
        ...
        {signups.length === 0 && (
          <Card className="p-8 text-center">
            <p className="text-grey-medium dark:text-gray-400">No signups found.</p>
          </Card>
        )}
      </div>
    </>
