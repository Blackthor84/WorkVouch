...
export function IdentitySimulatorPanel() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | undefined>(undefined);
  const [error, setError] = useState<string | undefined>(undefined);

  const spawn = useCallback(async (type: SpawnType) => {
    setLoading(true);
    setError(undefined);
    setResult(undefined);
    try {
  ...
  return (
    <div style={{ fontSize: 14 }}>
      <p style={{ margin: "0 0 8px 0", color: "#64748B" }}>Auto-verified fake users/employers. is_sandbox = true.</p>
      ...
      {result && <pre style={{ margin: 0, fontSize: 12, overflow: "auto", maxHeight: 120 }}>{result}</pre>}
    </div>
  );
}
