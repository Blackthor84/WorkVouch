...
export default function SelectRoleClient() {
  ...
  async function handleSelect(role: "user" | "employer") {
    ...
    setSelecting(role);
    try {
      const { error: profileError } = await supabase.from("profiles").update({ role: role === "employer" ? "employer" : "user" }).eq("id", user.id);
      if (profileError) {
      ...
      if (role === "employer") {
        try {
          ...
          if (planRaw) {
            ...
            if (plan?.plan_tier && ["starter", "pro", "custom"].includes(plan.plan_tier)) employerPlanTier = plan.plan_tier;
          }
        } catch {
          /* ignore */
        }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 px-4 overflow-x-hidden">
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg w-full max-w-md border border-gray-200 dark:border-gray-700">
        <p className="text-xs font-semibold uppercase tracking-wide text-[#64748B] mb-2 text-center">Step 2 of 3</p>
        <h1 className="text-2xl font-bold mb-2 text-center text-gray-900 dark:text-white">I am a:</h1>
        <p className="text-gray-600 dark:text-gray-400 text-sm text-center mb-6">You&apos;ll go straight to setup. No extra steps.</p>
        ...
        <div className="space-y-4">
          <button type="button" onClick={() => handleSelect("user")} disabled={!!selecting} className="w-full p-4 rounded-xl border-2 border-gray-200 dark:border-gray-600 hover:border-blue-500 dark:hover:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            {selecting === "user" ? "Setting up…" : "Employee"}
          </button>
          <button type="button" onClick={() => handleSelect("employer")} disabled={!!selecting} className="w-full p-4 rounded-xl border-2 border-gray-200 dark:border-gray-600 hover:border-blue-500 dark:hover:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            {selecting === "employer" ? "Setting up…" : "Employer"}
          </button>
        </div>
        <p className="text-center mt-6 text-sm text-gray-600 dark:text-gray-400">
          <Link href="/login" className="text-blue-600 dark:text-blue-400 hover:underline font-medium">Already have an account? Log in</Link>
        </p>
      </div>
    </div>
  );
